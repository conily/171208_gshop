...
<script>
  ...
  export default {
    ...
    methods: {
      _initScroll () {
        ...
        // 动态计算ul的宽度,因为ul的样式已经被固定了，所以无法形成滑动，需要通过js来修改ul的值形成滑动，也可以直接使用CSS将ul的宽度设置为所有图片的宽度来形成滑动
        const ul = this.$refs.picsUl
        const liWidth = 120
        const space = 6
        const count = this.info.pics.length
        ul.style.width = (liWidth + space) * count -space + 'px'//设置ul的宽度
  ...
  //如果从当前页面直接进行刷新时，有可能在执行_initScroll ()函数的时候，info还没有数据，就会报错，为了避免这种错误，需要监视info的值，让info有值以后再调用_initScroll ()函数。
    watch: {
      info () {// 刷新流程--> 更新数据
        this.$nextTick(() => {
          this._initScroll()
